...
const News = (props) => {
    ...
    const updateNews = async () => {
        ...
        const urls = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.catagory}&apiKey=${'e33f322b060b4ea7bbd2130ba7338a7a'}&page=${page}&pageSize=${props.pageSize}`;
        setLoading(true)
    ...
    const fetchMoreData = async () => {
        // this.setState({ page: this.state.page + 1 })
        const urls = `https://newsapi.org/v2/top-headlines?country=${props?.country}&category=${props?.catagory}&apiKey=${'e33f322b060b4ea7bbd2130ba7338a7a'}&page=${page+1}&pageSize=${props?.pageSize}`;
        setPage(page + 1)
        let data = await fetch(urls);
        let parsedData = await data.json();
